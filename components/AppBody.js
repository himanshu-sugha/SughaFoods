...
const Card = ({ name, rating, cuisines, costForTwo, cloudinaryImageId }) => {
    ...
    return (
        <div className="m-4 p-4 w-64 h-auto rounded-lg shadow-lg bg-white hover:shadow-xl hover:scale-105 transition duration-300 transform-gpu">
            <img
...
const promoted = (Card) => {
    return (props) => {
        return (
            <div>
                <label className="absolute  mx-4  rounded-md text-sm bg-red-600 text-white px-2 py-1  z-10">Closed</label>
                <Card {...props} />
            </div>
        )
    }
...
const PromotedRestCard = promoted(Card);
const AppBody = () => {
                    ...
                    <Link to={`/restaurants/${restaurant.info.id}`} key={index}>
                        {
                            ...
                            restaurant.info.isOpen ?
                                ...
                                />
                                :
                                <PromotedRestCard name={restaurant.info.name}
                                    rating={restaurant.info.avgRating}
                                    cuisines={restaurant.info.cuisines}
                                    costForTwo={restaurant.info.costForTwo}
                                    cloudinaryImageId={restaurant.info.cloudinaryImageId} />
